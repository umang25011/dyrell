...
function initCamera() {
  ...
  trackCamera["Ship"] = new cameraParameters(
    3.0 * celestialBodies["Ship"].radius,
    3.0 * celestialBodies["Ship"].radius,
    "Ship"
  )
}
...
function drawOrbit(celestialBody) {
  ...
  for (var i = 0; i <= radius; i++) {
    ...
    orbit.vertices.push(
      new THREE.Vector3(
        Math.cos(segment) * r * Math.cos(angle),
...
function initGui() {
  ...
  for (var i in calculateParams) calculate.add(calculateParams, i)
  ...
  comet
    .add(cometParams, "Length", 1000, 100000)
    .onChange(function () {
  ...
  comet
    .add(cometParams, "Size", 1000, 100000)
    .onChange(function () {
      window.removeEventListener("mousedown", onWindowMouseDown, false)
    })
    .onFinishChange(function () {
  ...
  for (var i in orbitParams) orbit.add(orbitParams, i)
  gui
    .add(params, "Camera", [
      "Galaxy",
      "Sun",
    ...
    this.Roam = function () {
      if (roamingStatus == false) {
        ...
        setTween(
          curBody,
          null,
          celestialBodies["Astronaut"].objectGroup.position.x,
          celestialBodies["Astronaut"].objectGroup.position.y,
          celestialBodies["Astronaut"].objectGroup.position.z
        )
        tween.start()
      } else {
        ...
        setTween(
          null,
          curBody,
          roamingCamera.camera.position.x,
          roamingCamera.camera.position.y,
          roamingCamera.camera.position.z
        )
        tween.start()
      }
    }
  ...
  gui
    .add(control, "TimeScale", 0.0, 10.0)
    .onChange(function (val) {
      window.removeEventListener("mousedown", onWindowMouseDown, false)
...
function init() {
  ...
  if (isTouchDevice) {
    window.addEventListener("touchstart", onWindowMouseDown, false)
    window.addEventListener("touchmove", onWindowMouseMove, false)
    window.addEventListener("touchend", onWindowMouseUp, false)
...
function onWindowResize() {
  ...
  if (roamingStatus) cameraControl.handleResize()
...
function checkCrash() {
  ...
  for (var objKey in celestialBodies) {
    if (celestialBodies[objKey].parent == celestialBodies["Sun"]) {
      ...
      if (Math.sqrt(dX * dX + dY * dY + dZ * dZ) > 2 * r) continue
      ...
      if (
        ...
        collisionResults[0].distance < celestialBodies[objKey].radius + directVector.length()
      ) {
        return true
      }
    }
